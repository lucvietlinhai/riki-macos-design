...
// Main component
interface ThumbPostTabProps {
    images: GeneratedImage[];
    onAddImages: (images: Omit<GeneratedImage, 'sourceTab'>[], sourceTab: 'thumbPost') => void;
...
export const ThumbPostTab: React.FC<ThumbPostTabProps> = (props) => {
    const { 
    ...
    const handleGenerate = useCallback(async () => {
        ...
        try {
            ...
            const newImages = results.map((base64, index) => ({
                ...
                x: 0, y: 0, z: Date.now() + index // Ensure unique z-index
            }));
            // Clear previous thumb post images before adding new ones
    ...
    }, [characterId, postContent, model, language, includeText, t, onAddImages, onDeleteImage, images]);
