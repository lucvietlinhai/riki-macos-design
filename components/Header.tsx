...
export const Header: React.FC<HeaderProps> = ({ onHistoryClick, onShortcutsClick, isProMode, model, onModelChange }) => {
  ...
  const iconColor = isProMode ? "text-orange-500 group-hover:text-orange-400" : "text-slate-700 dark:text-slate-300";
    ...
    <div className="relative flex items-center gap-2">
        ...
        <div className="relative">
            ...
            >
                ...
                <span className={`text-xs font-bold ${isProMode ? 'text-orange-400' : 'text-slate-600 dark:text-slate-300'}`}>
                    {model.includes('3.1') ? '3.1 Flash' : '2.5 Flash'}
                </span>
                <ChevronDownIcon className={`w-4 h-4 transition-transform ${isModelPopoverOpen ? 'rotate-180' : ''} ${isProMode ? 'text-orange-500' : 'text-slate-500 dark:text-slate-400'}`} />
            </button>

            {isModelPopoverOpen && (
                <div
                    ref={modelPopoverRef}
                    className={`absolute top-full right-0 mt-2 w-72 backdrop-blur-xl rounded-xl shadow-2xl p-2 border z-50 ${isProMode ? 'bg-zinc-950/95 border-orange-500/30' : 'bg-white/90 dark:bg-zinc-900/95 border-slate-200 dark:border-white/10'}`}
                >
                    <div className="px-2 py-1 mb-1">
                        <span className="text-[10px] font-black uppercase tracking-widest text-slate-400 dark:text-slate-500">{t('modelLabel')}</span>
                    </div>
                    <ul className="space-y-1">
                        <li>
                            <button
                                onClick={() => handleModelChange('gemini-2.5-flash-image')}
                                className={`w-full flex items-center justify-between p-2.5 rounded-lg text-left transition-all ${model === 'gemini-2.5-flash-image' ? (isProMode ? 'bg-orange-500/20 text-orange-200 border border-orange-500/30' : 'bg-indigo-50 dark:bg-indigo-500/10 text-indigo-700 dark:text-indigo-300') : 'hover:bg-black/5 dark:hover:bg-white/5 text-slate-600 dark:text-slate-400'}`}
                            >
                        ...
                        <li>
                            <button
                                ...
                                className={`w-full flex items-center justify-between p-2.5 rounded-lg text-left transition-all ${model === 'gemini-3.1-flash-image-preview' ? (isProMode ? 'bg-orange-500/20 text-orange-200 border border-orange-500/30' : 'bg-indigo-50 dark:bg-indigo-500/10 text-indigo-700 dark:text-indigo-300') : 'hover:bg-black/5 dark:hover:bg-white/5 text-slate-600 dark:text-slate-400'}`}
                            >
                                <div className="flex flex-col">
      ...
      <div className="relative">
          ...
          >
            ...
            <ChevronDownIcon className={`w-4 h-4 transition-transform ${isLangPopoverOpen ? 'rotate-180' : ''} ${isProMode ? 'text-orange-500' : 'text-slate-500 dark:text-slate-400'}`} />
          </button>
          ...
          >
              ...
              {(['vi', 'en', 'ja', 'id'] as Language[]).map((lang) => {
                  ...
                  <li key={lang}>
                      ...
                      className={`w-full flex items-center gap-2 p-1.5 rounded-md text-left text-sm transition-colors ${language === lang ? (isProMode ? 'bg-orange-600 text-black font-bold' : 'bg-indigo-600 text-white') : (isProMode ? 'text-orange-500/70 hover:bg-orange-500/20' : 'text-slate-700 dark:text-slate-300 hover:bg-black/10 dark:hover:bg-white/10')}`}
                      >
